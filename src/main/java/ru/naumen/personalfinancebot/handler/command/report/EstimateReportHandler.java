...
public class EstimateReportHandler implements CommandHandler {
    ...
    public EstimateReportHandler(ArgumentParseService argumentParseService, ReportService reportService) {
    ...
    public void handleCommand(CommandData data, Session session) {
        ...
        if (data.getArgs().isEmpty()) {
            ...
            try {
                yearMonth = this.argumentParseService.parseYearMonth(data.getArgs().get(0));
